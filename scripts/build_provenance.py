...
code_rows = [
    {
        "component": "transformer/runner.py",
        "provenance": "Adapted",
...
with open("code_provenance.csv", "w", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(
        f,
        fieldnames=["component", "provenance", "source_repo", "commit_sha", "license"],
    )
    w.writeheader()
...
with open("dataset_provenance.csv", "w", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(
        f, fieldnames=["artifact", "source", "time_window", "notes"]
    )
    w.writeheader()
    w.writerow(
        {
